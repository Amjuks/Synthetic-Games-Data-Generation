...
| `variant_scan_limit` | Number of deterministic candidates inspected before a finite route is treated as exhausted and cached. | int | `256` | positive integers | Optional |
| `allow_balanced_reuse` | Continue generation with the least-used valid variant after a finite route is exhausted. | bool | `true` | `true`, `false` | Optional |
