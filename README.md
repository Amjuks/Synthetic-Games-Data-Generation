...
Sudoku, KenKen, Kakuro, Star Battle, Nonogram, Hitori, and Nurikabe use the same multi-tool recording convention. Every generated record requires at least two distinct verified calls chosen for its task—for example rules plus a puzzle summary, constraint validation plus candidate analysis, or deduction analysis plus solution verification. Their schedulers cycle through easy, medium, hard, and expert positions; each puzzle records a deterministic complexity score, band, and measurable factors such as dimensions, clue density, and strategy count. Puzzle IDs from accepted and rejected history are reserved on resume, and selection refuses to reuse a puzzle within a job.
